...
export interface DuelRoom {
  id: string; // קוד חדר 4 ספרות (למשל: "4821")
  hostName: string;
  hostScore: number;
  hostHits: number;
  hostReady: boolean;
...
async function fetchRooms(): Promise<Record<string, DuelRoom>> {
  try {
    const res = await fetch(REGISTRY_ENDPOINT, { cache: 'no-store' });
    if (!res.ok) return {};
...
export async function createDuelRoom(hostName: string, totalPhrasesAvailable: number): Promise<DuelRoom> {
  ...
  const shortCode = Math.floor(1000 + Math.random() * 9000).toString(); // 4 ספרות
  ...
  // שמירה וניקוי חדרים מעל 30 דקות
  const now = Date.now();
  ...
  cleaned[shortCode] = roomData;

  // חובה להמתין לשמירה בענן לפני שמחזירים את החדר!
