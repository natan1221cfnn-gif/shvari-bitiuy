  ...
  useEffect(() => {
    ...
    טעןשיאיםמהענן().then((נתונים) => {
      if (!unmounted) {
        ...
        // 🔄 סנכרון דו-כיווני חכם: אם השחקן תואם לשיא בענן והשיא בענן גבוה יותר מהמקומי – עדכן את המכשיר שלו!
        const שיאבענןעבורשחקן = רשימהמשולבת.find((ש) => ש.שם === שםשחקן);
        if (שיאבענןעבורשחקן && שיאבענןעבורשחקן.ניקוד > שיאאישי) {
          localStorage.setItem('שברי-ביטוי-שיא', String(שיאבענןעבורשחקן.ניקוד));
          const מעודכן = [שיאבענןעבורשחקן, ...שיאים.filter((s) => s.ניקוד !== שיאבענןעבורשחקן.ניקוד)];
          localStorage.setItem('שברי-ביטוי-שיאים', JSON.stringify(מעודכן));
          useGameStore.setState({
            שיאאישי: שיאבענןעבורשחקן.ניקוד,
            שיאים: מעודכן,
          });
        }

        // אם יש לשחקן שיא אישי גבוה שלא מופיע ברשימת הענן, הוסף וסנכרן אותו
        if (שיאאישי > 0) {
