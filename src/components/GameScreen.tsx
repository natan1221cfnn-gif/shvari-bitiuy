...
export function GameScreen() {
  ...
  const handleAutoPass = useCallback(() => {
    ...
    // 💣 אם זו פצצה ועברה בלי לחיצה -> השחקן ניצל ולא מאבד לב!
    if (currSpecial === 'פצצה') {
      setSטטוסבלוק('הצלחה');
      setהודעה('כל הכבוד! נמנעת מהפצצה! 💣✅');
      נגןהצלחה();
      triggerJuice('success');
      // מעבר לביטוי הבא ללא איבוד לב!
      setTimeout(() => {
        setהודעה('');
        setSטטוסבלוק('ממתין');
        useGameStore.getState().עדכןביטוי();
      }, 700);
      return;
    }
  ...
  const startMove = useCallback(() => {
    ...
    const animate = (עכשיו: number) => {
      ...
      // חלון פגיעה מרווח והוגן (התקדמות בין 65% ל-115%)
      if (התקדמות >= 0.65 && התקדמות <= 1.15) {
  ...
  const handleTap = useCallback((e?: React.SyntheticEvent) => {
    ...
    // 🎯 1. חלון פגיעה מושלם ומרווח (בין 65% ל-115% מהמסלול)
    if (התקדמות >= 0.65 && התקדמות <= 1.15) {
    ...
    // 🤏 2. קרוב מאוד ("כמעט!") - ללא איבוד לב! (בין 48% ל-65%)
    if (התקדמות >= 0.48 && התקדמות < 0.65) {
      setSטטוסבלוק('כמעט');
