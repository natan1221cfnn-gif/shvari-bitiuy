...
export function LuckyWheelModal() {
  ...
  const סובבגלגל = () => {
    ...
    // חישוב זוית מדויקת ב-100% שנוחתת ישירות במרכז הפלח שנבחר מתחת למחוג העליון
    const יעדמודולו = (360 - (אינדקספרס * מעלותלפלח)) % 360;
    const זויתנוכחיתמודולו = ((סיבובזוית % 360) + 360) % 360;
    let הפרש = יעדמודולו - זויתנוכחיתמודולו;
    if (הפרש <= 0) הפרש += 360;
    const סיבוביםנוספים = 5 * 360;
    const זויתסופית = סיבובזוית + סיבוביםנוספים + הפרש;
